...
export class PortalProvider {
  ...
  render(component: React.ReactElement<any>, container: HTMLElement): void {
  ...
  subscribe(container: HTMLElement, cb: (data: any) => void): void {
    const set = this.nodeViewListeners.get(container) ?? new Set();
    set.add(cb);
    this.nodeViewListeners.set(container, set);
  }

  unsubscribe(container: HTMLElement, cb: (data: any) => void): void {
    const set = this.nodeViewListeners.get(container);
    if (!set) return;
    if (set.has(cb)) {
      set.delete(cb);
